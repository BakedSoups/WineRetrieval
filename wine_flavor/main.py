...
try:
    ...
    SIE_OCR_MODEL = _require_env("SIE_OCR_MODEL")
...
CUSTOM_RERANK_NO_REVIEW_PENALTY = float(
    os.getenv("CUSTOM_RERANK_NO_REVIEW_PENALTY"), 0.5
)
...
wines = datasource.attach_vivino_reviews(
    wines, review_pages=1, reviews_per_page=REVIEWS_PER_WINE, language="en"
)
...
top_matches = engine.cosine_similarity_search(
    user_vector, wine_matrix, top_k=COSINE_TOP_K
)
