...
type ResultsViewProps = {
  onBack: () => void
  filteredResults: RecommendedWine[]
  referenceWine: Wine | undefined
...
// ─── Profile form content (scrollable right panel) ──────────────────────────────

type ProfileFormContentProps = {
  isCatalogLoading: boolean
  isDetectingWine: boolean
  detectedWine: Wine | null
  detectionMatchScore: number | null
  selectedReferenceWine: string
  referenceWine: Wine | undefined
  wineSearchQuery: string
  wineSearchFocused: boolean
  filteredWineResults: Wine[]
  ...
  selectedFlavors: string[]
  errorMessage: string
  onPickCameraImage: () => void
  onPickPhotoLibraryImage: () => void
  onImageSelection: (e: React.ChangeEvent<HTMLInputElement>) => void
  onSelectWineFromSearch: (id: string) => void
  onClearReferenceWine: () => void
  onWineSearchQueryChange: (query: string) => void
  onWineSearchFocusChange: (focused: boolean) => void
  onStructureChange: (key: keyof WineStructure, value: number) => void
...
function ProfileFormContent({
  ...
  errorMessage,
  onPickCameraImage,
  onPickPhotoLibraryImage,
  onImageSelection,
  onSelectWineFromSearch,
  onClearReferenceWine,
  onWineSearchQueryChange,
  onWineSearchFocusChange,
  onStructureChange,
  onResetStructure,
  onFlavorToggle,
  onClearFlavors,
    ...
    <div className="max-w-2xl mx-auto px-8 py-8 space-y-6">
      ...
      <div className="rounded-2xl border border-border bg-card p-5">
        ...
        <div className="mb-4 grid gap-3 sm:grid-cols-2">
          ...
          <button
            onClick={onPickPhotoLibraryImage}
            disabled={isCatalogLoading || isDetectingWine}
        ...
        <div className="relative">
          <div className="relative">
            ...
            <Input
              ...
              onChange={(e) => onWineSearchQueryChange(e.target.value)}
              onFocus={() => onWineSearchFocusChange(true)}
              onBlur={() => setTimeout(() => onWineSearchFocusChange(false), 200)}
              ...
              {filteredWineResults.length > 0 ? (
                <div className="max-h-64 overflow-y-auto">
                  {filteredWineResults.map((wine) => (
                    <button
                      key={wine.id}
                      onClick={() => onSelectWineFromSearch(wine.id)}
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="relative flex-1 h-6 flex items-center">
                  ...
                  <input
                    ...
                    max="100"
                    value={structure[key]}
                    onChange={(e) => onStructureChange(key, Number(e.target.value))}
...
export function WineRecommender() {
  ...
  const handleClearReferenceWine = () => {
  ...
  const handlePickCameraImage = () => { cameraInputRef.current?.click() }
  const handlePickPhotoLibraryImage = () => { photoLibraryInputRef.current?.click() }
  ...
  const countries = [...new Set(recommendedWines.map((wine) => wine.country).filter(Boolean))] as string[]
  ...
  return (
    <div className="bg-background h-screen flex flex-col overflow-hidden">
      ...
      {/* Body: static left panel + scrollable right */}
      <div className="flex-1 overflow-hidden flex">
        {/* Left panel — static */}
        <div className="w-[340px] flex-shrink-0 border-r border-border px-8 py-8 space-y-6 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          <ProfileFormContent
            ...
            isDetectingWine={isDetectingWine}
            detectedWine={detectedWine}
            detectionMatchScore={detectionMatchScore}
            selectedReferenceWine={selectedReferenceWine}
            referenceWine={referenceWine}
            wineSearchQuery={wineSearchQuery}
            wineSearchFocused={wineSearchFocused}
            filteredWineResults={filteredWineResults}
            ...
            selectedFlavors={selectedFlavors}
            errorMessage={errorMessage}
            onPickCameraImage={handlePickCameraImage}
            onPickPhotoLibraryImage={handlePickPhotoLibraryImage}
            onImageSelection={handleImageSelection}
            onSelectWineFromSearch={handleSelectWineFromSearch}
            onClearReferenceWine={handleClearReferenceWine}
            onWineSearchQueryChange={setWineSearchQuery}
            onWineSearchFocusChange={setWineSearchFocused}
